...
export class RaCaptcha {
  ...
  public generate(
    length: number = 10,
    characters?: string,
    font: Font = { size: 35, font: 'conslas', style: 'bold' },
    lines: number = 5,
    dots: number = 30
  ) {
    this.context?.clearRect(0, 0, this.width, this.height);
    this.captcha = [];
    characters = characters || RaCaptcha.numbers;
  ...
  private randomLine(count: number = 5) {
    if (this.context)
      for (let i = 0; i < count; i++) {
        ...
        this.context.moveTo(this.randomNumber(this.width), this.randomNumber(this.height));
        this.context.lineTo(this.randomNumber(this.width), this.randomNumber(this.height));
        this.context.stroke();
      }
  }
