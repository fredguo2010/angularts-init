...
import { ALAIN_I18N_TOKEN, _HttpClient, SettingsService } from '@delon/theme';
...
export class DefaultInterceptor implements HttpInterceptor {
  ...
  constructor(private injector: Injector, private settingsService: SettingsService, private readonly keycloak: KeycloakService) {
  ...
  private refreshTokenRequest(): Observable<any> {
    ...
    const userid = this.settingsService.user.userid;
    const token = localStorage.getItem('_token');

    // console.log('keycloak');
    // this.keycloak.isLoggedIn().then(kLogin => {
    //   if (kLogin) {
    ...
    //console.log(userid);
    return this.http.post(`Users/RefreshToken`, null, { userid: userid }, { headers: { refresh_token: model?.['refreshtoken'] || '' } });
  }
  ...
  private tryRefreshToken(ev: HttpResponseBase, req: HttpRequest<any>, next: HttpHandler): Observable<any> {
    ...
    if ([`Users/RefreshToken`].some(url => req.url.includes(url))) {
      this.toLogin();
    ...
    return this.refreshTokenRequest().pipe(
      switchMap(res => {
        ...
        //console.log(res.user.expired);
        this.refreshToken$.next(res.user);
        // 重新保存新 token
